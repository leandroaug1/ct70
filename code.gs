...
function doPost(e) {
  try {
    ...
    return createJSONOutput({ success: false, error: "Erro no processamento JSON: " + error.message });
  }
}
...
function handleRequest(action, data) {
  ...
  try {
    ...
    let result;
    if (action !== 'GET_DASHBOARD') lock.tryLock(10000);

    switch (action) {
      case 'GET_DASHBOARD': result = service.getDashboardData(); break;
      case 'VALIDATE_TICKET': result = service.validarTickets(data.ids); break;
      case 'UPDATE_STATUS': result = service.atualizarStatus(data.id, data.status); break;
      case 'DELETE_TICKET': result = service.excluirTicket(data.id); break;
...
class CT70Service {
  ...
  getDashboardData() {
    ...
    if (idx.id === -1) throw new Error("Coluna identificadora (Ordem/OS) não encontrada.");
    ...
    return rows.map(r => {
      ...
      return {
        ...
        permanencia: val(idx.permanencia),
...
class Repository {
  ...
  findRowIndex(uid) {
    ...
    for (let i = 1; i < d.length; i++) { 
      if (String(d[i][col-1]).trim() == String(uid).trim()) return i + 1; 
    }
    return -1;
  }
  updateCell(id, col, cb) { 
    const r = this.findRowIndex(id);
    if (r > -1) { 
      const c = this.sheet.getRange(r, col); 
      c.setValue(cb(String(c.getValue()))); 
    }
  }
  deleteRow(id) { 
    const r = this.findRowIndex(id); 
    if (r > -1) this.sheet.deleteRow(r); 
  }
}
